...
## Worst-Case Latency Analysis of SEAM Synchronizer in ROS 
This is a **ROS2 Jazzy** package providing the evaluation test about the worst-case latency analysis for SEAM Synchronizer of [SEAM Synchronizer Policy](https://github.com/tianyiWangGithub/seam_synchronizer/message_filter/sync_policies/seam.h).
...
### Environment
- [Build ROS2 Jazzy in `ros2_Jazzy` workspace](https://docs.ros.org/en/Jazzy/Installation/Ubuntu-Development-Setup.html)
- Clone this project into your workspace (under 'src' directory)

### Files preparation

- Copy file in `message_filters/sync_policies` to `ros2_Jazzy/src/ros2/message_filters/include/message_filters/sync_policies`
- Copy file `msg/Timing.msg` to `/ros2_Jazzy/src/ros2/common_interfaces/std_msgs/msg/`
- Add the following line to file `/ros2_Jazzy/src/ros2/common_interfaces/std_msgs/CMakeLists.txt`, just after `set(msg_files`
    ```sh
    "msg/Timing.msg"
    ```
- Modify content of file `/ros2_Jazzy/src/ros2/common_interfaces/sensor_msgs/msg/JointState.msg` as follows:
    ```sh
...
cd ros2_Jazzy
source ./install/setup.bash
colcon build --packages-select seam_analysis --symlink-install
```

## Run evaluation

```sh
cd ~/ros2_Jazzy/src/seam_analysis
...
### Passing latency
```sh
cd ~/ros2_Jazzy/src/seam_analysis/scripts
